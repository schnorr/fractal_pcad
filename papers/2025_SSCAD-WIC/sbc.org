...
#+TITLE: Dynamic Load Balancing and Scalability Analysis of the Mandelbrot Set in a Multi-Thread HPC Application
...
indicate that the granularity factor has a major influence on load
balance, and its choice depends on the selected the fractal region.
#+end_abstract
...
performance analysis with metrics to provide overview and load
imbalance perspectives. Our main results observed in a cluster with a
W configuration is Z.
