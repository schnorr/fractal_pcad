...
- Client time: The total time taken for the client to receive the fully computed fractal
  for each setting.

- Speedup: The ratio of the baseline time to the time with multiple nodes, where the baseline
  is calculated as the average client time when using one node for the same case and granularity 
  setting. Note that the speedup is calculated relative to the number of nodes, not cores.
...
Note that the graphs for client time, speedup and efficiency report the mean across the 10
trials for each setting. Error bars indicate 95% confidence intervals for the mean.

#+CAPTION: Average client time for each setting.
#+NAME: fig:client-time
[[./figures/client_time.png]]

#+CAPTION: Average client speedup relative to 1 node for each setting.
#+NAME: fig:client-speedup
[[./figures/client_speedup.png]]

#+CAPTION: Average client efficiency relative to 1 node for each setting.
