...
~Discretize payloads~, which divides the requested region into several payloads, each covering 
...
are collected by the ~MPI Recv~ thread, which enqueues them into a response queue. These responses 
