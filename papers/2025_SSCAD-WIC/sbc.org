...
On the server side, the receiving thread listens for client payloads, forwarding them to the 
discretizing thread, which divides the requested region into several payloads, each covering 
a smaller square area according to the specified granularity. These are then placed into a queue, 
with outdated tasks and their pending responses being discarded to prevent workers from computing
regions that are no longer relevant to the client’s current view. As workers become available, they
request a new task from the coordinator. The MPI send thread dynamically assigns them new 
payloads from the discretizer's queue. Each worker independently computes its assigned payload, 
producing iteration counts for all pixels in that subregion. Once a block is completed, the results
are collected by the MPI receive thread, which enqueues them into a response queue. These responses 
are then collected by a network sender thread, which sends them back to the client. By returning 
