...
line. As such, the ~ProcessResponse~ thread is absent, and the ~Main~
...
setting. The *Speedup* measures the ratio of the mean Client time with 
a single node for a given region and granularity setting, to the mean Client 
time with multiple nodes for that same setting. That is, for a given number of 
nodes, $S(n) = \dfrac{T(1)}{T(n)}$. We emphasize that our speedup
...
We define the ideal speedup as: $S_{ideal}(n) = \dfrac{24n - 1}{23}$, and *Efficiency*
as: $E(n) = \dfrac{S(n)}{S_{ideal}(n)}$
This ensures that perfect linear scaling as workers are added results in efficiency = $1.0$, 
...
for higher node counts at values over 40. This effect is higher the more nodes, and therefore workers, 
are present. The /default/ case in particular seems to suffer from more worker imbalance
...
region. Our analysis showed that while the system scales well for
compute-intensive workloads with balanced granularity, communication
overhead can dominate in simpler cases, reducing efficiency. These
insights highlight the importance of tuning granularity based on
