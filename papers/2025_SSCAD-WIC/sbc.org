...
- **Number of Server Nodes:** [1, 2, 3, 4, 5, 6] nodes were tested, each contributing
  24 physical cores. This allows us to measure how well the system scales with more resources.
...
- Client time: The average total time taken for the client to receive the fully computed fractal
  for each setting.

- Speedup: The ratio of the baseline time (average single-node client time for each granularity 
  value) to the time with multiple nodes.

- Efficiency: The speedup normalized by the number of nodes, indicating how well the system scales.

- Imbalance Percentage: ...

#+CAPTION: Client time, speedup and efficiency for each node/granularity configuration
#+NAME: fig:client-metrics
[[./figures/client_metrics.png]]

Examining Figure [[fig:client-metrics]], 
...
between the payload size and the number of payloads assigned to each worker, keeping a low 
communication overhead while also balancing work between workers well.
...
As such, lower granularities lead to higher overhead, which grows worse as more nodes are added. 
This effect is especially visible at granularity 5: in the /default/ and /hard/ cases, 
